...
onready var option = preload("res://galgame/scenes/ui/option.tscn")
...
func _ready():
	...
	galgame_reader.play_dialog(global_var.current_story)


func _exit_tree():
	signal_mgr.unregister("setname", self, "_on_set_name")
	signal_mgr.unregister("setsprite", self, "_on_set_sprite")
	signal_mgr.unregister("settext", self, "_on_set_text")
	signal_mgr.unregister("setoption", self, "_on_set_option")
	signal_mgr.unregister("setfinsh", self, "_on_set_finsh")
...
func _on_set_text(text):
	...
	global_var.dotween(dialog_label, "percent_visible", 0, 1, 1)
